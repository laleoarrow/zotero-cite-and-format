...
3. Verify target-journal rules from official sources for manuscript font, abstract length, title page, line numbering, table placement, figure legends, figure file delivery, and supplementary files. Citation storage remains fixed to dynamic Zotero fields. If the journal or submission system requires field-code removal or rejects live fields, record the exact conflict as a blocker rather than creating a static copy. If other required rules are not found, report "official instructions silent/not found."
...
5. Verify Zotero integrity: every visible citation is backed by a live citation field; every visible Zotero-managed reference list is backed by one continuous live bibliography field; document preferences exist; field result text matches stored Zotero citation text; and no orphaned typed citations, static reference blocks, or duplicate reference blocks remain.
...
- Never create or deliver a static, unlinked, or flattened citation DOCX. If the user requests additional Word variants, keep Zotero citation and bibliography fields dynamic in every variant.
